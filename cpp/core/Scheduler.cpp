...
void Scheduler::simulate_severity_update(int zone_id, int new_severity) {
    ...
    const Depot fallback = {-1, "unassigned", zone.lat, zone.lng, 0, 0, 0, false};
    const Depot& priority_source = nearest != nullptr ? *nearest : fallback;
    const double new_key = compute_priority_key(zone, priority_source);

    if (zone.is_served) {
        zone.is_served = false;
        zone.assigned_depot = -1;
        if (zone.food_needed <= 0) {
            auto food_it = data_->base_food_needed.find(zone.id);
            zone.food_needed = food_it != data_->base_food_needed.end()
                                   ? std::max(1, food_it->second)
                                   : 1;
        }
        if (zone.medicine_needed <= 0) {
            auto med_it = data_->base_medicine_needed.find(zone.id);
            zone.medicine_needed = med_it != data_->base_medicine_needed.end()
                                       ? std::max(1, med_it->second)
                                       : 1;
        }
        if (!data_->priority_queue) {
            data_->priority_queue = std::make_unique<FibonacciHeap>();
        }
        zone.heap_node = data_->priority_queue->insert(zone.id, new_key);
    } else if (zone.heap_node == nullptr) {
        if (!data_->priority_queue) {
            data_->priority_queue = std::make_unique<FibonacciHeap>();
        }
        zone.heap_node = data_->priority_queue->insert(zone.id, new_key);
    } else if (new_key < zone.heap_node->key) {
        data_->priority_queue->decrease_key(zone.id, new_key);
    } else if (new_key > zone.heap_node->key) {
        data_->priority_queue->delete_node(zone.id);
        zone.heap_node = data_->priority_queue->insert(zone.id, new_key);
    }
